...
## Работа STP на разных уровнях

### STP и уровни модели OSI

- **STP работает на канальном уровне (L2)** — оперирует MAC-адресами и BPDU (Bridge Protocol Data Units).  

- **Но зависит от физической топологии (L1)** — если кабель оборван или порт выключен, STP не сможет построить дерево.  

Почему это важно?

- STP **логически** блокирует петли (на канальном уровне), но **физически** соединение остаётся (порт в состоянии *Blocking* всё ещё получает BPDU).  

- Если физическая топология не соответствует логической (например, кабель подключён, но STP его "не видит"), возможны проблемы.  


### Как STP взаимодействует с физикой?

- **Пример 1**:  

  - **Физическая петля**: Два коммутатора соединены двумя кабелями. 
   
  - **STP-реакция**: Один порт перейдёт в состояние *Blocking*, чтобы избежать петли на канальном уровне.  

- **Пример 2 (проблема)**:  

  - Если **физическое соединение есть**, но **STP не работает** (например, отключён на одном коммутаторе) — возникнет broadcast-шторм.  


### Где STP "не видит" физику?

- **Unidirectional links** (односторонние кабели):  

  - Кабель передаёт данные только в одну сторону (например, повреждён). 
   
  - STP может не обнаружить проблему, так как BPDU не доходят, и порт остаётся в *Forwarding* $\rightarrow$ петля!  
  
  - **Решение**: Использовать **Loop Guard** или **UDLD** (протоколы для обнаружения таких ситуаций).  
  
: Работа STP {#tbl:L2}

| **Аспект**       | **STP (L2)**               | **Физический уровень (L1)**          |
|-------------------|------------------------------------|--------------------------------------|
| **Что контролирует?** | Петли на уровне MAC-адресов       | Целостность кабеля, сигнал          |
| **Инструменты**   | BPDU, блокировка портов           | LED-индикаторы, тестеры кабелей     |
| **Если ошибка**   | Порты блокируются                 | Связь пропадает полностью            |

**STP (Spanning Tree Protocol)** работает на **втором уровне модели OSI (канальный уровень - Data Link Layer)**.  

- **Канальный уровень (L2)** отвечает за передачу кадров (frames) между устройствами в одной сети (LAN).  

- **STP** используется для предотвращения петель (loops) в сетях Ethernet, управляя избыточными соединениями между коммутаторами (L2-устройствами).  

- **STP оперирует BPDU (Bridge Protocol Data Units)**, которые передаются между коммутаторами для построения дерева без петель.  

Почему не другие уровни?

- **Физический уровень (L1)** – STP не работает с битами и физическими соединениями.  

- **Сетевой уровень (L3) и выше** – STP не использует IP-адреса или транспортные протоколы.  

**Вывод:** STP – это **чисто канальный (L2) протокол**.
...
Чтобы подробно разобрать разновидности STP (Spanning Tree Protocol) и их отличия, рассмотрим каждый вариант с техническими особенностями, сферами применения и ключевыми различиями.  

### 1. CST (Common Spanning Tree)

- Описание:  

  - Один экземпляр STP для всей сети, независимо от количества VLAN.  
  
  - Все VLAN используют одно общее дерево (одна корневая коммутатор, одни блокируемые порты).  
  
- Особенности:  

  - Простота настройки, но неэффективность в сетях с множеством VLAN.  
  
  - Если топология меняется, весь STP пересчитывается, что может вызвать задержки. 
   
- **Где применяется**:  

  - В устаревших сетях или оборудовании, не поддерживающем VLAN-зависимые версии STP.  

### 2. PVST+ (Per-VLAN Spanning Tree Plus)

- Описание:  

  - Каждый VLAN имеет собственное дерево STP. 
   
  - Развитие Cisco (проприетарный протокол), основанное на стандартном STP (IEEE 802.1D).  
  
- Особенности:  

  - Позволяет балансировать нагрузку между VLAN (разные корневые коммутаторы для разных VLAN).
    
  - Требует больше ресурсов CPU и памяти, так как для каждого VLAN строится отдельное дерево.  
  
  - Использует ISL (устаревший) или 802.1Q для передачи BPDU.  
  
- **Отличие от CST**:  

  - В CST — одно дерево на всю сеть, в PVST+ — отдельное дерево для каждого VLAN.  

### 3. RSTP (Rapid Spanning Tree Protocol, IEEE 802.1w)
 
- Описание:  

  - Улучшенная версия STP с ускоренной сходимостью (1-2 секунды вместо 30-50 у STP).  
  
  - Обратная совместимость с STP (может работать в одном домене с классическим STP).  
  
- Особенности:  

  - Вводит новые роли портов: *Alternate* (резервный) и *Backup* (дублирующий).  
  
  - Упрощённый процесс перехода в состояние Forwarding (нет состоятий Listening, Blocking заменено на Discarding). 
   
  - BPDU теперь обрабатываются как обычные данные (в STP — только корневой коммутатор генерирует BPDU).  
  
- **Отличие от CST/PVST+**:  

  - Быстрая реакция на изменения топологии.  
  
  - В RSTP нет отдельных деревьев для VLAN (если не используется RPVST+ от Cisco).  
...
- Описание:  
  ...
  - Объединяет несколько VLAN в одну STP-инстанцию (MSTI — MST Instance). 
   
  - Позволяет балансировать нагрузку без создания отдельного дерева для каждого VLAN.  
  
- Особенности: 
 
  - VLAN группируются в регионы (MST Region) с одинаковыми настройками.  
  
  - Экономит ресурсы коммутаторов (меньше BPDU, чем в PVST+).  
  
  - Совместим с RSTP (использует его механизмы сходимости).  
  
- **Отличие от PVST+/RSTP**:  

  - В PVST+ — отдельное дерево на VLAN, в MSTP — несколько VLAN в одном дереве.  
  
  - В RSTP — одно дерево для всех VLAN, в MSTP — гибкая группировка.  

### Сравнительная таблица

: Разновидности STP {#tbl:STP}

| **Характеристика** | **CST** | **PVST+** | **RSTP** | **MSTP** |
|---------------------|---------|-----------|----------|----------|
| **Количество деревьев** | Одно на всю сеть | Одно на VLAN | Одно (если не RPVST+) | Группы VLAN (MSTI) |
| **Сходимость** | Медленная (30-50 сек) | Медленная (как STP) | Быстрая (1-2 сек) | Быстрая (как RSTP) |
| **Балансировка нагрузки** | Нет | Да (разные корневые коммутаторы) | Нет (если не RPVST+) | Да (гибкая группировка) |
| **Ресурсы** | Минимальные | Высокие (много BPDU) | Умеренные | Оптимизированные |
| **Стандарт** | IEEE 802.1D | Cisco (на основе 802.1D) | IEEE 802.1w | IEEE 802.1s |
